// Signature Packs UI — import, browse, scan, and apply signature packs.
// Extracted from monolith index.html v0.9.31.

let signaturePackManager;
let spectrum;
let labelManager;
let regionManager;
let showMessage;
...
async function scanGitHubRepo(ownerRepo, path, depth, sigGitHubStatus) {
    ...
    for (const subdir of subdirs) {
        ...
        const subResults = await scanGitHubRepo(ownerRepo, subdir, depth + 1, sigGitHubStatus);
...
function renderGitHubFileList(sigGitHubFileList, sigGitHubActions, sigGitHubSelected) {
    ...
    for (let i = 0; i < sigGitHubFiles.length; i++) {
        ...
        cb.addEventListener('change', () => {
            ...
            updateGitHubSelectedCount(sigGitHubSelected);
        });
...
function updateGitHubSelectedCount(sigGitHubSelected) {
...
export function initSignaturesUI(deps) {
    signaturePackManager = deps.signaturePackManager;
    ...
    document.getElementById('btnSigGitHubFetch').addEventListener('click', async () => {
        ...
        try {
            sigGitHubFiles = await scanGitHubRepo(ownerRepo, '', 0, sigGitHubStatus);
            sigGitHubStatus.textContent = `Found ${sigGitHubFiles.length} file(s) in ${ownerRepo}`;
            renderGitHubFileList(sigGitHubFileList, sigGitHubActions, sigGitHubSelected);
        } catch (e) {
